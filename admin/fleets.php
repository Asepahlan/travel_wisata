        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Armada</th>
...
// Tampilkan modal edit
function editFleet(id, nama, nomorPolisi, jenis, kapasitas, keterangan, status) {
    document.getElementById('modalTitle').textContent = 'Edit Armada';
    document.getElementById('formAction').value = 'edit';
    document.getElementById('fleetId').value = id;
    document.getElementById('nama_armada').value = nama;
    document.getElementById('nomor_polisi').value = nomorPolisi;
...
// Konfirmasi hapus
function confirmDelete(id, nama) {
    if (confirm(`Apakah Anda yakin ingin menghapus armada "${nama}"?`)) {
        ...
        .then(data => {
            if (data.success) {
                ...
                alert(data.message || 'Gagal menghapus armada');
            }
        })
        .catch((error) => {
            console.error('Error:', error);
            alert('Terjadi kesalahan saat menghapus armada');
        });
    }
}
